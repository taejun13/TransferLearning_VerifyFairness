...
from datasets import AdultDataset, CompasDataset
...
def main(args):
    ...
    if args.dataset == 'adult':
        data['train'] = AdultDataset(train=True)
        data['test'] = AdultDataset(train=False)
        ...
        data['train'] = CompasDataset(train=True)
        data['test'] = CompasDataset(train=False)
    ...
    test_loader = DataLoader(data['test'], batch_size=args.batch, shuffle=False) 
        ...
        for method in methods:
            for lamb in lambdas:
                ...
                if args.dataset=='adult':
                    ...
                    args.epochs = 20 if lamb in [0.1, 1.0] else 15
                
                t1 = time.time()
                print(f'====== Trial={n}|lamb={lamb}|method={method}========')
                performance = train_nets(args, loaders, method, proportions, lamb=lamb)
                print(f'>>>TOOK {round(time.time()-t1,3)} secs')
                for key in performance.keys():
                    if key not in results[m_lamb].keys():
                        results[m_lamb][key] = [performance[key]]
                    else:
                        results[m_lamb][key].append(performance[key])
    ...
    save_txt = f'results_{args.dataset}.txt'
    ...
    # save json
    import json
    save_json = f'results_{args.dataset}_trial_5_new_fair.json'
...
def train_nets(args, loaders, method, prop, lamb):
    ...
    for epoch in range(args.epochs): 
        ...
        for x, y, a in loaders['train_loader']:
            x = x.cuda()
            y = y.cuda()
            a = a.cuda()
        ...
        for x, y, a in loaders['test_loader']:
            with torch.no_grad():
                x = x.cuda()
                y = y.cuda()
                a = a.cuda()
