...
class CustomDataset(Dataset):
    ...
    def get_A_proportions(self):
        A0 = (self.A==0).sum() / self.A.shape[0]
        A1 = (self.A==1).sum() / self.A.shape[0]
        assert A0 + A1 == 1
        return [A0, A1]

    def get_Y_proportions(self):
        Y0 = (self.Y==0).sum() / self.Y.shape[0]
        Y1 = (self.Y==1).sum() / self.Y.shape[0]
        assert Y0 + Y1 == 1
        return [Y0, Y1]

    def get_YA_proportions(self):
        Y0A0 = (self.A==0)[self.Y==0].sum()
        Y1A0 = (self.A==0)[self.Y==1].sum()
        Y0A1 = (self.A==1)[self.Y==0].sum()
        Y1A1 = (self.A==1)[self.Y==1].sum()
        ...
        Y1A0 /= num_A0
        Y1A1 /= num_A1
        return [[Y0A0, Y1A0], [Y0A1, Y1A1]]

class HealthDataset(CustomDataset):
    """
    Heritage Health dataset.
    """
    def __init__(self, stage='test'):
        self.data_path = 'data/health.npz'
        self.all_data = np.load(self.data_path)
        self.length = len(self.all_data['X'])
        

        if stage=='train':
            start_idx = 0
            end_idx = 60000
        elif stage=='transfer_train':
            start_idx = 60000
            end_idx = 76000
        elif stage=='transfer_test':
            start_idx = 76000
            end_idx = 80000
        elif stage=='test':
            start_idx = 60000
        ...
        self.X = self.all_data['X'][start_idx:end_idx, :] 
        self.Y = self.all_data['Y'][start_idx:end_idx] 
        self.A = self.all_data['A'][start_idx:end_idx] 
        self.PCG = self.all_data['PCG'][start_idx:end_idx, :] 
    ...
    def __getitem__(self, idx):
        ...
        Y = torch.tensor(self.Y[idx])
        A = torch.tensor(self.A[idx])
        PCG = torch.tensor(self.PCG[idx])
        return X, Y, A, PCG



class CompasDataset(CustomDataset):
    """
    COMPAS dataset.
    """
    def __init__(self, train=False):
        self.data_path = 'data/compas.csv'
        self.all_data = pandas.read_csv(self.data_path).values
        self.data = self.all_data[:4320, :] if train else self.all_data[4320:,]

        # Data, label and sensitive attribute partition.
        self.X = self.data[:, 1:].astype(np.float32)
        self.Y = self.data[:, 0].astype(np.int64)
        self.A = self.data[:, 5].astype(np.int64)


class AdultDataset(CustomDataset):
    def __init__(self, train=False):
        super(AdultDataset, self).__init__(train)
        self.data = np.load('data/adult.npz')
        if train:
        ...
        self.A = np.argmax(self.A, axis=1)
...
class Dataset(object):
    ...
    def get_YA_proportions(self):
        ...
        Y0A0 = TN(self.attr_train, self.y_train)
        Y0A1 = FN(self.attr_train, self.y_train)
        Y1A0 = FP(self.attr_train, self.y_train)
        Y1A1 = TP(self.attr_train, self.y_train)
