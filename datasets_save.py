...
class AdultDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # convert onehot into label index
        Y = torch.argmax(Y)
        A = torch.argmax(A)
        return X, Y, A
    ...
    # borrowed from lafter
    def get_A_proportions(self):
        A = np.argmax(self.A, 1)
        A0 = (A==0).sum() / A.shape[0]
        A1 = (A==1).sum() / A.shape[0]
        assert A0 + A1 == 1
        return [A0, A1]

    def get_Y_proportions(self):
        Y = np.argmax(self.Y, 1)
        Y0 = (Y==0).sum() / Y.shape[0]
        Y1 = (Y==1).sum() / Y.shape[0]
        assert Y0 + Y1 == 1
        return [Y0, Y1]

    def get_AY_proportions(self):
        ttl = float(self.Y.shape[0])
        A0Y0 = TN(self.Y, self.A) / ttl
        A0Y1 = FN(self.Y, self.A) / ttl
        A1Y0 = FP(self.Y, self.A) / ttl
        A1Y1 = TP(self.Y, self.A) / ttl
...
class Dataset_cfair(Dataset):
    def __init__(self, **args):
        super(Dataset_cfair, self).__init__(**args)
    ...
    def get_batch_iterator(self, phase, mb_size):
        ...
        sz = x.shape[0]
        batch_inds = make_batch_inds(sz, mb_size, self.seed, phase)
        iterator = DatasetIterator([torch.tensor(x, dtype=torch.float32), torch.tensor(y, dtype=torch.float32), torch.tensor(a, dtype=torch.float32)], batch_inds)
        return iterator
